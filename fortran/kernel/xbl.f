C Extracted kernel source. Original source: vendor/xfoil/src/xbl.f
...
C---- check for transition and set TRAN, XT, etc. if found
      ...
      IF(IBL.EQ.ITRAN(IS) .AND. .NOT.TRAN) THEN
       WRITE(99,*) 'SETBL: Xtr???  n1 n2: ', AMPL1, AMPL2
      ENDIF
C
...
C
      IF(TFORCE(IS)) THEN
       WRITE(99,9100) IS,XOCTR(IS),ITRAN(IS)
 9100  FORMAT(1X,'Side',I2,' forced transition at x/c = ',F7.4,I5)
      ELSE
       WRITE(99,9200) IS,XOCTR(IS),ITRAN(IS)
