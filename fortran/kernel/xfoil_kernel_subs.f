C Extracted kernel source. Original source: vendor/xfoil/src/xfoil.f
...
C
      IF(RETYP.LT.1 .OR. RETYP.GT.3) THEN
        WRITE(99,*) 'MRCL:  Illegal Re(CL) dependence trigger.'
        WRITE(99,*) '       Setting fixed Re.'
        RETYP = 1
      ENDIF
      IF(MATYP.LT.1 .OR. MATYP.GT.3) THEN
        WRITE(99,*) 'MRCL:  Illegal Mach(CL) dependence trigger.'
        WRITE(99,*) '       Setting fixed Mach.'
        MATYP = 1
      ENDIF
C
C
...
C
      IF(MINF .GE. 0.99) THEN
        WRITE(99,*)
        WRITE(99,*) 'MRCL: CL too low for chosen Mach(CL) dependence'
        WRITE(99,*) '      Aritificially limiting Mach to  0.99'
        MINF = 0.99
        M_CLS = 0.
      ENDIF
C
...
C
      IF(RRAT .GT. 100.0) THEN
        WRITE(99,*)
        WRITE(99,*) 'MRCL: CL too low for chosen Re(CL) dependence'
        WRITE(99,*) '      Aritificially limiting Re to ',REINF1*100.0
        REINF = REINF1*100.0
        R_CLS = 0.
      ENDIF
C
...
C
      IF(DENNEG) THEN
       WRITE(99,*)
       WRITE(99,*) 'CPCALC: Local speed too large. ',
...
C
      ...
      YBTE = 0.5*(YB(1) + YB(NB))
C
      WRITE(99,1050) XBLE,YBLE, CHORDB,
     &              XBTE,YBTE
C
...
C
      IF(ITYPE.EQ.0) THEN
       WRITE(99,*) 'This designation not implemented.'
       RETURN
      ENDIF
C
...
C
      IF(NB.LT.2) THEN
       WRITE(99,*) 'PANGEN: Buffer airfoil not available.'
       N = 0
       RETURN
      ENDIF
C
...
C------ update node position
        ...
        ENDDO
C
CCC        IF(RLX.EQ.1.0) WRITE(99,*) DMAX
CCC        IF(RLX.NE.1.0) WRITE(99,*) DMAX,'    RLX =',RLX
        IF(ABS(DMAX).LT.1.E-3) GO TO 11
   10 CONTINUE
      WRITE(99,*) 'Paneling convergence failed.  Continuing anyway...'
C
   11 CONTINUE
C
...
ccc      WRITE(99,*) 'DSmin/DSavg = ',DSMIN,'     DSmax/DSavg = ',DSMAX
C
...
C
      IF(SHARP) THEN
       WRITE(99,1090) 'Sharp trailing edge'
      ELSE
       GAP = SQRT((X(1)-X(N))**2 + (Y(1)-Y(N))**2)
       WRITE(99,1090) 'Blunt trailing edge.  Gap =', GAP
      ENDIF
 1090 FORMAT(/1X,A,F9.5)
C
      IF(SHOPAR) WRITE(99,1100) NPAN, CVPAR, CTERAT, CTRRAT,
