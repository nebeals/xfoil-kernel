C Extracted kernel source. Original source: vendor/xfoil/src/xpanel.f
...
C-    (fraction of smaller panel length adjacent to TE)
      BWT = 0.1
C
      WRITE(99,*) 'Calculating unit vorticity distributions ...'
C
      DO 10 I=1, N
        GAM(I) = 0.
        GAMU(I,1) = 0.
...
C-----------------------------------------------------
      INCLUDE 'XFOIL.INC'
C
      WRITE(99,*) 'Calculating source influence matrix ...'
C
      IF(.NOT.LADIJ) THEN
C
...
C-----------------------------------------------------
      INCLUDE 'XFOIL.INC'
C
      WRITE(99,*) 'Calculating wake trajectory ...'
C
...
c      write(99,*) waklen, chord, waklen*chord
c      write(99,*) ds1
c      do i = n+1, n+nw
c        write(99,*) i-n, snew(i)
c      enddo
...
C
   ...
   10 CONTINUE
C
      WRITE(99,*) 'STFIND: Stagnation point not found. Continuing ...'
      I = N/2
C
   11 CONTINUE
C
...
C
      ...
      IF(IBLMAX.GT.IVX) THEN
        WRITE(99,*) ' ***  BL array overflow.'
        WRITE(99,*) ' ***  Increase IVX to at least', IBLMAX
        STOP
      ENDIF
C
      LIPAN = .TRUE.
...
C
      ELSE
C
CCC       WRITE(99,*) 'STMOVE: Resetting stagnation point'
C
...
c        write(99,*) 'Uenew xinew', idif+1, uedg(idif+1,2), xssi(idif+1,2)
